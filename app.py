...
st.caption("2892 Benda Uji | Live Demo untuk Teknisi Lab")

@st.cache_data
def load_data():
    # Cari file xlsx APAPUN yang ada - anti error nama _x5f_
    files = glob.glob("*.xlsx") + glob.glob("*.xls")
    # Prioritaskan file yang mengandung qc_clean
    files_sorted = sorted(files, key=lambda x: (0 if "qc" in x.lower() else 1, -os.path.getsize(x) if os.path.exists(x) else 0))
    st.sidebar.write(f"File terdeteksi: {files_sorted}")
    ...
    for fname in files_sorted:
        try:
            ...
            used = fname
            st.sidebar.success(f"Pakai: {fname} ({len(df)} baris)")
            break
        except Exception as e:
            st.sidebar.error(f"Gagal baca {fname}: {e}")
            continue
    
    if df is None or df.empty:
        st.error("File Excel tidak ketemu! Pastikan upload qc_clean_GITHUB_DEMO.xlsx")
        return pd.DataFrame()
    return df
